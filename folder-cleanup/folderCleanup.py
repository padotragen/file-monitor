...
RUMBAFOLDER = os.getenv("SOURCE_DIR", "/app/MonitoredFolder")
MAXFOLDERSIZE = float(os.getenv("MAXFOLDERSIZE", 2))
TARGETFOLDERSIZE = float(os.getenv("TARGETFOLDERSIZE", 1))
CLEANUPTIMEOUT = float(os.getenv("CLEANUPTIMEOUT", 30))
...
if __name__ == "__main__":
    targetFolder = FolderCleanup(RUMBAFOLDER)

    while True:
        try:
            directorysizeMB = round(targetFolder.get_dir_size() / (1024 * 1024), 2)
            ...
            if directorysizeMB > MAXFOLDERSIZE:
                ...
                targetFolder.remove_files_until_size(TARGETFOLDERSIZE)  # Reduce to TARGETFOLDERSIZE
            time.sleep(CLEANUPTIMEOUT)  # Wait for 30 seconds before running again
